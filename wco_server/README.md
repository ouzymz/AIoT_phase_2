...
Also supports direct upload to Google Drive for Phase 2 training data collection.
...
├── token.pickle                   # OAuth token cache (auto-generated on first run)
├── routers/
│   ├── upload.py                  # POST /upload, POST /uploadGoogleDrive
│   │                              # GET /stats, GET /apply-thresholds
│   │                              # POST /revert-names
...
├── services/
│   ├── drive.py                   # Google Drive upload via OAuth
...
Create a `.env` file in `wco_server/`:

```env
GDRIVE_FOLDER_ID=<your-google-drive-folder-id>
GOOGLE_OAUTH_CREDENTIALS_JSON=/absolute/path/to/client_secret_*.json
```

- `GDRIVE_FOLDER_ID` — ID of the Google Drive folder to upload images into (from the folder URL).
- `GOOGLE_OAUTH_CREDENTIALS_JSON` — absolute path to the OAuth 2.0 client secret JSON downloaded from Google Cloud Console (Desktop app type).

### 3. First run — OAuth consent

On the first request to `/uploadGoogleDrive`, a browser window will open for Google OAuth consent. After authorization, a `token.pickle` file is created next to `main.py` and reused for subsequent requests. Tokens are refreshed automatically when expired.
...
On startup the server prints its LAN IP and all endpoint URLs, and reports calibration status.
...
| `POST` | `/uploadGoogleDrive` | Upload a JPEG directly to Google Drive with explicit labels. |
...
**`POST /uploadGoogleDrive`** — multipart form fields:

| Field | Type | Description |
|-------|------|-------------|
| `file` | JPEG | Image file |
| `turbidity` | int (0 or 1) | Turbidity label |
| `particle` | int (0 or 1) | Particle label |
| `color` | int (0 or 1) | Color change label |

Filename is derived from labels: `t{turbidity}-p{particle}-c{color}.jpeg`
...
Labeled images saved by `/upload` follow:
...
Images uploaded via `/uploadGoogleDrive` use dashes and no index:

```
t{0|1}-p{0|1}-c{0|1}.jpeg
```

Uncalibrated `/upload` images are saved as `raw_<UTC-timestamp>.jpg` with empty label columns.
