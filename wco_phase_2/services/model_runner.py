...
def model_footprints() -> list[dict]:
    """
    Return static (request-independent) footprint metrics for each loaded model:
    on-disk file size and trainable parameter count. Used by GET /models/footprint
    to compare model sizes without running inference.
    """
    footprints = []
    for name, cfg in MODEL_CONFIGS.items():
        file_path = MODELS_DIR / cfg["file"]
        file_bytes = file_path.stat().st_size
        params = _loaded_models[name].count_params() if name in _loaded_models else None
