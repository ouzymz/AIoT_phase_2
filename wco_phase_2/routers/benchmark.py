...
from services.model_runner import available_models, model_footprints, run_benchmark, run_single
...
class FootprintResponse(BaseModel):
    models: list[ModelFootprint]


@router.get("/models/footprint", response_model=FootprintResponse)
async def models_footprint():
    """
    Static (request-independent) computing footprint of the three models:
    on-disk file size and trainable parameter count. Returns the same numbers
    on every call — no inference is performed.
    """
    return FootprintResponse(models=model_footprints())
